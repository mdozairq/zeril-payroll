...
AlgoKit workspace with **three** applications:

```
giggling/
├── projects/
│   ├── payroll-contracts/     # Employer smart contract (PuyaTs) + Vitest E2E
│   ├── payroll-api/           # Express + Prisma (KYC, runs, tax, off-ramp, auth)
│   └── payroll-frontend/      # React + Vite + typed contract clients
├── docs/
│   ├── ARCHITECTURE.md        # System diagram + scaling notes
│   ├── CODE_QUALITY.md        # Repo structure + conventions
│   ├── SECURITY.md          # Threat model + known gaps
│   └── TECHNICAL_PANEL.md     # Judge demo / walkthrough cheat sheet
├── .github/workflows/         # CI for contracts, frontend, API
└── .algokit.toml
```
...
API listens on `http://localhost:3001` (`GET /api/health`).

### 7. Run the frontend

```bash
cd projects/payroll-frontend
# Set VITE_API_URL=http://localhost:3001 in .env if needed
npm run dev
```

Open the app in your browser, connect a wallet, and follow the setup wizard (Deploy > Initialize > Bootstrap).

For **technical judging**, see [docs/TECHNICAL_PANEL.md](./docs/TECHNICAL_PANEL.md).
...
|----------|---------|---------|
| `validate.yaml` | PR | Runs contracts + frontend + API CI |
| `payroll-contracts-ci.yaml` | PR (via validate) | Lint, test, TEAL audit, LocalNet deploy |
| `payroll-api-ci.yaml` | PR (via validate) | Type-check, build, API tests |
| `payroll-frontend-ci.yaml` | PR (via validate) | Lint, test, build frontend |
| `payroll-contracts-cd.yaml` | Release | Deploy contracts to TestNet |
| `payroll-frontend-cd.yaml` | Release | Deploy frontend |
...
| Component | Host |
|-----------|------|
| Frontend | **Vercel** — `projects/payroll-frontend` |
| API | **Render** — `projects/payroll-api` + PostgreSQL |
| Contracts | **Algorand TestNet** — GitHub Actions / AlgoKit |
